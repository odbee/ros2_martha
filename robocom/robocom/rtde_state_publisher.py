...
class RTDEStatePublisher(Node):
    ...
    def timer_callback(self):
        try:
            ...
            if state is not None:
                ...
                if input_bits != self.cachedinput:
                    self.cachedinput=input_bits
                    self.get_logger().info(' pins: %s' % input_bits  )


                
                
                self.active=int(input_bits[2])
                self.referenced=int(input_bits[3])
    ...
    def move_axis(self, order):
        ...
        self.get_logger().error("sending order %s " % order)           
