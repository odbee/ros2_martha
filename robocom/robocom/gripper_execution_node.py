...
class GripperControlServer(Node):
    ...
    ######################### START GLOBAL GRIPPER VARIABLES
    """
    ...
    class ObjectStatus(IntEnum):
        ...
        AT_DEST = 3
    ########################### END GLOBAL GRIPPER VARIABLES

    ########################### START GRIPPER FUNCTIONS
    ...
    def is_active(self):
        ...
        return GripperControlServer.GripperStatus(status) == GripperControlServer.GripperStatus.ACTIVE
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        if GripperControlServer.ObjectStatus(status) != GripperControlServer.ObjectStatus.AT_DEST:
    ...
    def move_and_wait_for_pos(self, position: int, speed: int, force: int) -> Tuple[int, ObjectStatus]:  # noqa
        ...
        while GripperControlServer.ObjectStatus(cur_obj) == GripperControlServer.ObjectStatus.MOVING:
            cur_obj = self._get_var(self.OBJ)

        # report the actual position and the object status
        final_pos = self._get_var(self.POS)
        final_obj = cur_obj
        return final_pos, GripperControlServer.ObjectStatus(final_obj)
    ...
    def __init__(self):
        ...
        self._min_force = 0
        self._max_force = 255
        ############################## END INIT GRIPPER VARIABLES

        super().__init__('gripper_control_server')
        self.srv=self.create_service(MoveGripper, 'move_gripper',self.move_gripper_callback)
    ...
    def move_gripper_callback(self,request,response):
        ...
        elif(request.command=='m'):

            if (self._get_var(self.ACT)==1):
                lastPos, status= self.move_and_wait_for_pos(request.position,request.speed,request.force)
                response.returnpos= lastPos
                response.objectstatus=int(status)
            else:
                self.get_logger().info("Failed to move. activating gripper then trying to move again")
                self.activate()
                lastPos, status= self.move_and_wait_for_pos(request.position,request.speed,request.force)
                response.returnpos= lastPos
                response.objectstatus=int(status)
                response.returncode=0
        elif(request.command=='r'):
            self._reset()
            response.returncode=0
...
def main(args=None):
    ...
    gripper_control_server.connect('192.168.1.104',63352)
    gripper_control_server.get_logger().info('successfully connected to robot')
    rclpy.spin(gripper_control_server)
